...
/// </para>
/// </summary>
public sealed record ServerOptions(IReadOnlyList<string> Roots, bool StrictConventions, string? FollowFile = null, string? SelectFile = null)
{
    ...
    public static ServerOptions Parse(IReadOnlyList<string> args)
    {
        ...
        var select = Environment.GetEnvironmentVariable("AML_MCP_SELECT");
        ...
        for (var i = 0; i < args.Count; i++)
        {
            switch (args[i])
            {
                ...
                // A file an editor watches: the server writes into it which element to show.
                case "--select" when i + 1 < args.Count && !args[i + 1].StartsWith("--", StringComparison.Ordinal):
                    select = args[++i];
                    break;
                case "--select":
                    throw new ArgumentException("--select needs a file.");
        ...
        return new ServerOptions(normalized, strict,
            ...
            string.IsNullOrWhiteSpace(select) ? null : Path.GetFullPath(select.Trim().Trim('"')));
    }
